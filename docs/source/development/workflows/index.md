...
Its intended level is application-operation orchestration, not automatic acceleration of tight loops or replacement of OpenMP, oneTBB, SIMD, or optimized numerical libraries. Those facilities can provide kernel-level parallelism inside a workflow job. See {doc}`Intended scope and granularity <../parallel_execution/scope_and_granularity>` for the comparison and composition guidance.
